...
public class JedisAdapter implements InitializingBean {
    ...
    public void afterPropertiesSet(){
        try {
            ...
            pool = new JedisPool(new JedisPoolConfig(),redisHost, 6379,8000,  "snnuPai&+1");

//            System.out.println(redishost);
            logger.info("redis start successfull!");
        }catch (Exception e){
    ...
    public void sadd(int eventType, int entityType, String entityId, int secondEntityType, String secondEntityId){
        Jedis jedis = null;
        try {
            jedis = pool.getResource();
            String key = RedisKeyUtil.getKey(eventType, entityType, entityId);
            String val = RedisKeyUtil.getVal(secondEntityType, secondEntityId);
            String secondKey = RedisKeyUtil.getKey(eventType, secondEntityType, secondEntityId);
            String secondVal = RedisKeyUtil.getVal(entityType, entityId);
            boolean isMember = jedis.sismember(key, val);
            if (isMember) {
                jedis.srem(key, val);
            } else {
                jedis.sadd(key, val);
            }
            boolean isSecondMember = jedis.sismember(secondKey, secondVal);
            if (isSecondMember) {
                jedis.srem(secondKey, secondVal);
            } else {
                jedis.sadd(secondKey, secondVal);
            }
        }catch (Exception e){
